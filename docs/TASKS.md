...
- [x] Fix silent Supabase error in `src/app/search/page.tsx` — `fetchPerformances` has no try/catch; a thrown error leaves `loading:false` with empty results and zero user feedback; add try/catch and a `searchError` state that shows an inline error banner beneath the search box.
